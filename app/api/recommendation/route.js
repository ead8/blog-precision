...
export async function GET(request) {
  try {
    ...
    // Get all search params as filters
    const filters = {};
    ...
    if (id) {
      const data = okOrThrow(
        await supabaseAdmin.from('recommendations').select('*').eq('id', id).single()
      );
      return NextResponse.json(data);
    }
    
    let q = supabaseAdmin.from('recommendations').select('*');
...
export async function POST(request) {
  try {
    ...
    const data = okOrThrow(
      await supabaseAdmin.from('recommendations').insert(body).select('*').single()
    );
