  ...
  try {
    ...
    if (id) {
      const data = okOrThrow(
        await supabaseAdmin.from('generated_articles').select('*').eq('id', id).single()
      );
      return NextResponse.json(data);
    }
    
    let q = supabaseAdmin.from('generated_articles').select('*');
...
export async function POST(request) {
  try {
    ...
    const data = okOrThrow(
      await supabaseAdmin.from('generated_articles').insert(body).select('*').single()
    );
